...
\end{align*}\right.
$$
解得$\hat E(t,\xi)=\dfrac{\sin(a|\xi|t)}{a|\xi|\sqrt{2\pi}^n}=\dfrac{t}{\sqrt{2\pi}^n}\sum_{k\geq 0}\dfrac{(-a^2\xi^2 t^2)^{k}}{(2k+1)!}$​. Fourier逆变换得
$$
\begin{align*}
E(t,x)=\dfrac{t}{{2\pi}^n}\sum_{k\geq 0}\dfrac{(-a^2t^2)^k}{(2k+1)!}\prod_{d=1}^n\int_{\mathbb R}e^{(\xi_d^2)^k}e^{i\xi_d x_d}\mathrm d\xi_d.
\end{align*}
$$
对低维简单情形, 可直接求解. 
